...
*Setup instructions for a Gitian build of Bit Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bit
...
- [Building Bit Core](#building-Bit-core)
...
Clone the git repositories for Bit and Gitian.
...
in the Bit repository under 'Fetch and create inputs' to install sources which require
...
To build Bit Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Bit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Bit-project/Bit
...
For example:
```bash
URL=https://github.com/thrasher-/Bit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Bit=${COMMIT} --url Bit=${URL} ../Bit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Bit=${COMMIT} --url Bit=${URL} ../Bit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Bit=${COMMIT} --url Bit=${URL} ../Bit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Bit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Bit/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Bit
SIGPATH=/some/root/path/Bit-detached-sigs

./bin/gbuild --url Bit=${BTCPATH},signature=${SIGPATH} ../Bit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Bit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Bit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Bit-osx-build.assert
```
...
[Bit-project/gitian.sigs.BCT](https://github.com/Bit-project/gitian.sigs.BCT/) repository, or if that's not possible create a pull
