...
*Setup instructions for a Gitian build of Bitcoin2 Core using a Debian VM or physical system.*
...
- [Building Bitcoin2 Core](#building-Bitcoin2-core)
...
Clone the git repositories for Bitcoin2 and Gitian.
...
in the Bitcoin2 repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bitcoin2 Core
----------------

To build Bitcoin2 Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bitcoin2 repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/Bitcoin2/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/Bitcoin2-project/Bitcoin2
...
For example:
```bash
URL=https://github.com/thrasher-/Bitcoin2.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit Bitcoin2=${COMMIT} --url Bitcoin2=${URL} ../Bitcoin2/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit Bitcoin2=${COMMIT} --url Bitcoin2=${URL} ../Bitcoin2/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit Bitcoin2=${COMMIT} --url Bitcoin2=${URL} ../Bitcoin2/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the Bitcoin2 git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../Bitcoin2/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/Bitcoin2
SIGPATH=/some/root/path/Bitcoin2-detached-sigs

./bin/gbuild --url Bitcoin2=${BTCPATH},signature=${SIGPATH} ../Bitcoin2/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/Bitcoin2-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/Bitcoin2-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/Bitcoin2-osx-build.assert
```
...
[Bitcoin2-project/gitian.sigs.BCT](https://github.com/Bitcoin2-project/gitian.sigs.BCT/) repository, or if that's not possible create a pull
