Bitcoin2 Core version 0.13.2 is now available from:

  <https://download.bitcoin2.org/bitcoin2-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/bitcoin2-project/bitcoin2/issues>
...
updates will be released anymore. Without security updates, using a bitcoin2
...
ECDSA signatures inside Bitcoin2 transactions now use validation using
...
Previous versions of Bitcoin2 Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bitcoin2 Core was to raise the default minimum
relay fee.

Bitcoin2 Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bitcoin2 Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running bitcoin2d without having to do any manual configuration.
...
Bitcoin2 Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bitcoin2 Core will relay transactions with
...
entirely in Bitcoin2 Core 0.13, and it is at this time undecided whether the
...
Bitcoin2 Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bitcoin2 Core automatically creates a hidden service to listen on, without
manual configuration. Bitcoin2 Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bitcoin2 Core is listening, and
...
Bitcoin2d can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bitcoin2
Core to use floating fees. By default, Bitcoin2 Core will use floating
fees.
...
At all times, Bitcoin2 Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BCB.
Furthermore, Bitcoin2 Core will never create transactions paying less than
...
`bitcoin2.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /bitcoin2rpc>
        ...
        # optional bypass bitcoin2d rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< bitcoin2rpc:<password>
    </Location>
...
Restarting bitcoin2d will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in bitcoin2.conf
...
bitcoin2-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitcoin2-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bitcoin2 Core code base has
...
- `bitcoin2-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bitcoin2-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bitcoin2 scripting system that in combination with
...
Bitcoin2 transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bitcoin2 Core.
...
The two phases are distinct in the Bitcoin2-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bitcoin2 wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bitcoin2.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bitcoin2 block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of bitcoin2s they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of bitcoin2s being spent is exactly
...
- **Increased security for multisig:** Bitcoin2 addresses (both P2PKH addresses
  ...
  believed to be provided by Bitcoin2's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bitcoin2's current ledger (called the UTXO set),
  ...
  note that Bitcoin2 Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bitcoin2 users to individually opt-in to almost any change in the Bitcoin2
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bitcoin2 Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bitcoin2
client to Bitcoin2 Core 0.13.2 to signal for segwit support. When a
...
Since Bitcoin2 Core 0.10.0, nodes have defaulted to only relaying and
...
- `bitcoin2-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `bitcoin2-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
